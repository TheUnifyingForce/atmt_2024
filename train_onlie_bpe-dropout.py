...
############## BPE ##########
def apply_bpe_dropout(args, epoch):
    """ Apply BPE-dropout to the training data before each epoch. """
    logging.info(f"Applying BPE-dropout for epoch {epoch}")
    ...
    call(
        f"cat {src_bpe_file} | python {bpe_scripts}/apply_bpe.py -c {src_bpe_file} --dropout {dropout_prob} > {src_bpe_applied}",
        shell=True)
    call(
        f"cat {tgt_bpe_file} | python {bpe_scripts}/apply_bpe.py -c {tgt_bpe_file} --dropout {dropout_prob} > {tgt_bpe_applied}",
        shell=True)

    logging.info(f"BPE-dropout applied for epoch {epoch}")
############################################
...
def get_args():
    ...
    parser.add_argument('--bpe-dropout', default=0.1, type=float, help='BPE-dropout probability')
