...
class BeamSearch(object):
    ...
    def get_best(self):
        ...
        node = merged.get()
        return node[0], node[2]  # score and node
    ...
    def prune(self):
        ...
        # Prune based on the highest probability of finished hypotheses
        best_final_score = float('-inf')  # Track the best final score (highest probability)
        if finished > 0:
            best_final_score = self.final.queue[0][0]  # Top of the final queue

        for _ in range(self.beam_size - finished):
            node = self.nodes.get()
            # Keep nodes with scores higher than the best finished hypothesis
    ...
    def prune_incomplete(self):
        """ Prunes incomplete nodes with lower scores than the best finished one """
        nodes = PriorityQueue()
        finished_score = None
        if self.final.qsize() > 0:
            finished_score = self.final.queue[0][0]  # The score of the best finished hypothesis

        # Only keep nodes whose score is higher than the best finished hypothesis
        while not self.nodes.empty():
            score, _, node = self.nodes.get()
            if finished_score is None or score > finished_score:
